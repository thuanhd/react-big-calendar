...
function getEventResizeTimes(event, value, startAccessor) {
  // calculate new start/end when resizing an event

  // resizing down
  var start = (0, _accessors.accessor)(event, startAccessor);
  var end = value;

  end = (0, _moment2.default)(start).set({
    hour: end.getHours(),
...
function createWrapper(type) {
  ...
  var dropTarget = {
    ...
    hover: function hover(_, monitor, _ref2) {
      ...
      if (!window.reactBigCalendarResizerValue || window.reactBigCalendarResizerValue.toString() !== value.toString()) {
        ...
        if (itemType === 'event-resize') {
          ...
          propsCallback = { event: event, value: value };
        }
  ...
  return (0, _reactDnd.DropTarget)(['event', 'event-resize'], dropTarget, collectTarget)(DraggableBackgroundWrapper);
}
